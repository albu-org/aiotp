...
from ..abstracts import AbstractHOTP


class HOTP(AbstractHOTP, OTP):
    def __init__(
        self,
        secret: str,
        digits: int = 5,
    ...
    async def __aenter__(self) -> 'HOTP':
        return self
    ...
    async def create(self, count: int) -> str:
        ...
        Returns:
            otp-code -> str
        """
        return await self._generate(self.count + count)
    ...
    async def verify(self, code: str, counter: int) -> bool:
        ...
        s_counter = unicodedata.normalize('NFKC', await self.create(counter))
